...
|      | Serverless 应用引擎 SAE                | 是一个全托管、免运维、高弹性的通用 PaaS 平台。SAE 支持 Spring Boot、Spring Cloud、Dubbo、HSF、Web 应用和 XXL-JOB、ElasticJob 任务的全托管，零改造迁移、无门槛容器化、并提供了开源侧诸多增强能力和企业级高级特性。                                     | 微服务应用托管；弹性阔缩容场景；持续集成与交付；                                         | 贵州酒店集团 视野数科 爱奇艺体育               | 类似 side car ，用来管理应用，承接流量 |
